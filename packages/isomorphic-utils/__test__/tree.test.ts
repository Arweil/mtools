...
describe('tree', () => {
  test('simple2Tree', () => {
    expect(
      simple2Tree<{ id: number; pId: number; name: string }>({
  ...
  test('tree2Simple', () => {
    expect(
      tree2Simple<{ id: number; pId: number; name: string }>({ tree: cloneInDeep(treeData) }),
    ).toEqual(cloneInDeep(simpleData));
  });

  test('levelOrder, forEachTree', () => {
    const copyTreeData = cloneInDeep(treeData);
    forEachTree(copyTreeData, item => {
