...
function simple2Tree<T extends IBaseSimpleTreeData>(data: {
  ...
  const { simpleData = [], pIdKey = 'pId', idKey = 'id', pIdStart = 0 } = data;
 ...
 * 树形结构的数据转换为简单的数据格式
 */
function tree2Simple<T>(data: { tree: typeBaseTreeNode<T>; pIdKey?: string; idKey?: string }): T[] {
...
export { simple2Tree, tree2Simple, levelOrder, forEachTree };
