...
export function getURLParams<T = { [key: string]: string }>(fullPath: string): T {
  ...
  return Object.fromEntries(search.join('').split('&').map((item) => item.split('=')));
}
 ...
 * @returns
 */
export function makeURLParams(params: { [key: string]: string | boolean | number | undefined | null }) {
 ...
 * @returns
 */
export function addURLParams(url: string, params: { [key: string]: string | boolean | number | undefined | null }) {
