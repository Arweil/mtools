...
export interface MapTheme {
  theme?: Theme;
  className?: string;
  emotioncss?: Partial<Record<Theme, ((token: GlobalToken, prefixCls: string, tokenExt: Partial<ThemeColor>) => string) | undefined>>;
...
export default function useMapTheme(props: MapTheme) {
  ...
  const mergedColor = useMemo(() => mergeColor(hermesDefaultColor, tokenExt || {}), [hermesDefaultColor, tokenExt]);
  ...
  const classes = useMemo(
    () => (
      {
        hermes: classNames(className, [emotioncss && emotioncss.hermes && emotioncss.hermes(token, prefix, mergedColor)]),
        zeus: classNames(className, [emotioncss && emotioncss.zeus && emotioncss.zeus(token, prefix, mergedColor)]),
        default: classNames(className, [emotioncss && emotioncss.default && emotioncss.default(token, prefix, mergedColor)]),
